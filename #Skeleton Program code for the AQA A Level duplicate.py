...
class Company:
  ...
  def GetDetails(self):
    ...
    if self._Balance > 0:
      Details += f"Current balance: {self._Balance}  \nAverage cost per meal: {self._AvgCostPerMeal:.2f}\n"
    else:
      Details += f"Current balance: BANKRUPT  \nAverage cost per meal: {self._AvgCostPerMeal:.2f}\n"
    Details += f"Average price per meal: {self._AvgPricePerMeal: .2f}  \nDaily costs: {self._DailyCosts: .2f} \n"
    Details += f"Delivery costs: {self.CalculateDeliveryCost(): .2f} \nReputation: {self._ReputationScore: .2f}\n\n"
    Details += f"Number of outlets: {len(self._Outlets): .2f} \n Outlets\n"
  ...
  def ProcessDayEnd(self):
    ...
    if self._Balance > 0:
      Details += f"New balance for company: {self._Balance :.2f}"
    else:
      Details += "New balance for company: BANKRUPT"
    return Details
